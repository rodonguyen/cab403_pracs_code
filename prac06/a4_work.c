...
void *consumer(void *arg)
{
  ...
  {
    pthread_mutex_lock(&mutex);
    while (consume_pointer == produce_pointer) // while there's nothing in the buffer
    {                                   
      pthread_cond_wait(&full, &mutex); // wait until there is an "full" buffer, while waiting, unlock the mutex for other threads
    }
