...
int main(int argc, char **argv){
	...
	socklen_t addr_size;
	char *ip = "127.0.0.1";
	char *endNarration = "Closing connection to the server.\n";
	char *stdNarration = "Server says: Message received successfully\n";
	...
	*/	
	...
	while (cont) {	
		...
		bzero(buffer, BUFFER_SIZE);
		if (cont) {
			strcpy(buffer, stdNarration);
			
		} else {
			strcpy(buffer, endNarration);
